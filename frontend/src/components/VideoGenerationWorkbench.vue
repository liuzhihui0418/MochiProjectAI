...
/>
    ...
    <GlobalConfirm />
    <!-- ================= 顶部导航 ================= -->
    <header class="h-20 px-6 border-b border-white/5 bg-gradient-to-b from-black/90 via-[#05050a]/90 to-black/80 backdrop-blur-xl flex justify-between items-center shrink-0 z-50 relative">
      <!-- 霓虹边框效果 -->
       ...
       <!-- 模型配置按钮 & 弹出面板 -->
        <div class="relative z-50">
          ...
          <button
            ...
            class="relative px-5 py-2.5 rounded-lg transition-all duration-300 group overflow-hidden border border-cyan-500/20 hover:border-cyan-400/60 bg-gradient-to-br from-[#111118] to-[#0a0a12] hover:from-[#1a1a2a] hover:to-[#0f0f1a] min-w-[180px]"
          >
            <!-- 按钮内容 -->
            <div class="relative z-10 flex items-center justify-between gap-3">
               ...
               <div class="flex items-center gap-3">
                  <!-- 图标容器：增加发光底座 -->
                  <div class="relative p-1 rounded-md bg-cyan-500/10 border border-cyan-500/20 group-hover:border-cyan-400/50 transition-colors">
                     <Settings :size="18" class="text-cyan-400 group-hover:rotate-90 transition-transform duration-500" />
                  </div>

                  <div class="flex flex-col items-start text-left">
                     <span class="text-sd text-cyan-100 font-bold group-hover:text-white transition-colors tracking-wide">高级模型配置</span>
                  </div>
               </div>

               <!-- 箭头 -->
               <ChevronDown :size="14" class="text-cyan-500/60 group-hover:text-cyan-300 transition-colors duration-300" :class="showModelConfig ? 'rotate-180' : ''" />
            </div>

            <!-- 流光背景特效 (复刻右侧按钮) -->
            <div class="absolute inset-0 bg-gradient-to-r from-cyan-500/0 via-cyan-500/10 to-cyan-500/0 -translate-x-full group-hover:translate-x-full transition-transform duration-700 ease-in-out"></div>

            <!-- 底部发光线条 -->
            <div class="absolute -bottom-[1px] left-4 right-4 h-[1px] bg-gradient-to-r from-transparent via-cyan-400 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 shadow-[0_0_10px_#22d3ee]"></div>
          </button>

          <!-- 2. 弹出的详细配置面板 (流光边框炸裂版) -->
          <div
            v-if="showModelConfig"
            class="absolute top-16 left-0 z-[100] animate-in fade-in slide-in-from-top-2 duration-300"
          >

            <!-- 🔥🔥🔥 核心：流光边框容器 🔥🔥🔥 -->
            <div class="relative p-[2px] rounded-xl overflow-hidden group/panel">
                <!-- 动态旋转的渐变背景 (模拟边框) -->
                <div class="absolute inset-0 bg-gradient-to-r from-cyan-500 via-purple-500 to-cyan-500 animate-gradient-xy opacity-100"></div>

  <!--
    外层容器：用于实现流光边框效果
    原理：背后的 div 拥有多彩渐变并旋转，通过 blur 扩散光晕
  -->
  <div class="relative group w-[600px] rounded-[12px]">

    <!-- 动态流光背景 (Animated Glow) -->
    <div class="absolute -inset-[1px] bg-gradient-to-r from-cyan-500 via-purple-500 to-pink-500 rounded-[12px] opacity-30 blur-sm group-hover:opacity-100 transition duration-1000 group-hover:duration-200 animate-tilt"></div>

    <!-- 主体面板 -->
    <div class="relative bg-[#09090b] rounded-[12px] overflow-hidden shadow-2xl ring-1 ring-white/10">
...
<div class="flex items-center gap-3 w-full">
    ...
    <!-- 1. 左侧：独立输入框 (自动撑满剩余宽度) -->
   <div class="flex-1 relative bg-[#131318] rounded-xl flex items-center shadow-inner shadow-black/50 transition-all focus-within:shadow-[inset_0_2px_10px_rgba(0,0,0,1)]">
    ...
    <!--
      👇 重点在这里 👇
      1. outline-none: 去掉浏览器默认白框
      2. border-none: 去掉边框
      3. focus:ring-0: 去掉Tailwind默认蓝框
      4. appearance-none: 去掉系统默认外观
    -->
    <input
        type="text"
        placeholder="请输入您的API激活卡密"
        class="w-full bg-transparent border-none outline-none focus:outline-none focus:ring-0 appearance-none text-sd font-bold text-gray-200 placeholder-gray-600 px-3 py-2 font-mono tracking-wide"
    />

</div>

    <!-- 2. 右侧：独立的量子流光按钮 -->
    <!-- 注意：removed mr-1 because gap-3 handles spacing now -->
    <button class="relative group/btn inline-flex items-center justify-center rounded-lg cursor-pointer transition-transform active:scale-95 shrink-0">

        <!-- 旋转能量边框 (核心科技) -->
        <div class="relative rounded-lg p-[1px] overflow-hidden bg-[#09090b]">
            <!-- 旋转的背景层 -->
            <div class="absolute inset-[-1000%] bg-[conic-gradient(from_90deg_at_50%_50%,#09090b_0%,#10b981_50%,#09090b_100%)] animate-[spin_2s_linear_infinite]"></div>

            <!-- 按钮主体 (黑色内芯) -->
            <div class="relative flex items-center gap-2 px-5 py-2.5 rounded-[7px] bg-[#09090b]/90 backdrop-blur-xl group-hover/btn:bg-[#09090b]/80 transition-colors border border-white/5">

                <!-- 文字：带发光阴影 -->
               <span class="text-sd leading-none font-bold bg-gradient-to-r from-emerald-400 to-cyan-300 bg-clip-text text-transparent group-hover/btn:text-white group-hover/btn:drop-shadow-[0_0_10px_rgba(52,211,153,0.8)] transition-all duration-300">
    验证卡密
</span>

                <!-- 图标：带高频闪烁 (已帮您修正 size 为 14 以便显示) -->
                <div class="relative">
                    <Zap :size="14" class="text-emerald-500 group-hover/btn:text-cyan-300 transition-colors" />
                    <Zap :size="14" class="absolute inset-0 text-white opacity-0 group-hover/btn:animate-ping" />
                </div>
            </div>
        </div>
    </button>
        ...
        <!-- 第一行: 语言模型 (Cyan Theme) -->
        <div class="bg-[#131318]/80 backdrop-blur-sm rounded-xl p-4 border border-white/5 hover:border-cyan-500/40 hover:bg-[#131318] transition-all duration-300 group/item hover:shadow-[0_0_15px_-3px_rgba(6,182,212,0.15)]">

  <div class="flex items-center justify-between mb-4">
    <span class="text-gray-400 text-sd font-bold w-20 shrink-0 group-hover/item:text-cyan-300 transition-colors">改文模型：</span>

    <div class="flex-1 mx-4 relative group/select">
      <div class="flex items-center justify-between bg-[#0a0a0f] border border-white/10 rounded-lg px-3 py-2.5 cursor-pointer hover:border-cyan-500/50 hover:shadow-[0_0_10px_rgba(34,211,238,0.15)] transition-all">
        <span class="text-cyan-100 text-sd font-bold tracking-tight">deepseek-v3-250324</span>
        <ChevronDown :size="18" class="text-gray-500 group-hover/select:text-cyan-400 transition-colors" />
      </div>
    </div>
  </div>
  ...
  <div class="flex items-center">
    ...
    <div class="flex-1 mx-4 flex items-center justify-between">
      ...
      <label class="inline-flex items-center cursor-pointer group/toggle relative">
        <input type="checkbox" class="sr-only peer">
        <span class="ms-2 text-xs font-bold text-gray-500 group-hover/toggle:text-gray-300 transition-colors select-none"></span>
      </label>

      <button class="group/edit relative px-4 py-1.5 rounded-lg bg-transparent overflow-hidden transition-all hover:shadow-[0_0_15px_rgba(34,211,238,0.3)]">
         <div class="absolute inset-0 border border-white/10 rounded-lg group-hover/edit:border-cyan-400/50 transition-colors"></div>
         <div class="absolute inset-0 bg-cyan-500/0 group-hover/edit:bg-cyan-500/10 transition-colors"></div>
         <div class="flex items-center gap-2 relative z-10 text-gray-400 group-hover/edit:text-cyan-300 transition-colors text-sd font-bold">
            <span>编辑提示词</span>
            <ArrowRight :size="12" class="opacity-0 -translate-x-2 group-hover/edit:opacity-100 group-hover/edit:translate-x-0 transition-all duration-300" />
         </div>
      </button>
       ...
       <!-- 第二行: 分镜改写 (Purple Theme) - 已添加卡密输入 -->
<div class="bg-[#131318]/80 backdrop-blur-sm rounded-xl p-4 border border-white/5 hover:border-purple-500/40 hover:bg-[#131318] transition-all duration-300 group/item hover:shadow-[0_0_15px_-3px_rgba(168,85,247,0.15)]">

  <!-- 1. 顶部: 模型选择 -->
  <div class="flex items-center justify-between mb-3">
    <span class="text-gray-400 text-sd font-bold w-20 shrink-0 group-hover/item:text-purple-300 transition-colors">推理提示词</span>

    <div class="flex-1 mx-4 relative group/select">
      <div class="flex items-center justify-between bg-[#0a0a0f] border border-white/10 rounded-lg px-3 py-2.5 cursor-pointer hover:border-purple-500/50 hover:shadow-[0_0_10px_rgba(168,85,247,0.15)] transition-all">
        <span class="text-purple-100 text-sd font-bold tracking-tight">gemini-3-pro-preview</span>
        <ChevronDown :size="18" class="text-gray-500 group-hover/select:text-purple-400 transition-colors" />
      </div>
    </div>
  </div>

  <!-- 2. 中间: 新增卡密输入区域 (无边框沉浸式 + 紫色炸裂按钮) -->
  <div class="flex items-center mb-3">
    <!-- 占位符，为了让输入框跟上面的下拉框对齐 (w-20对应上面的label宽度) -->
    <div class="w-20 shrink-0"></div>
    ...
    <div class="flex-1 mx-4 flex items-center gap-2">
        ...
        <!-- 左侧: 独立输入框 -->
        <div class="flex-1 relative bg-[#0a0a0f] rounded-lg flex items-center shadow-inner shadow-black/50 transition-all focus-within:shadow-[inset_0_2px_10px_rgba(0,0,0,1)]">
            <div class="pl-3 text-gray-500 group-focus-within:text-purple-500 transition-colors">
                <Key :size="18" />
            </div>
            <!-- 严选去边框样式 -->
            <input
                type="text"
                placeholder="请输入推理提示词卡密"
                class="w-full bg-transparent border-none outline-none focus:outline-none focus:ring-0 appearance-none text-sd font-bold text-gray-200 placeholder-gray-600 px-3 py-2  tracking-wide"
            />
        </div>
        ...
        <button class="relative group/btn inline-flex items-center justify-center rounded-lg cursor-pointer transition-transform active:scale-95 shrink-0">
            <!-- 旋转能量边框 (紫色) -->
            <div class="relative rounded-lg p-[1px] overflow-hidden bg-[#09090b]">
                <div class="absolute inset-[-1000%] bg-[conic-gradient(from_90deg_at_50%_50%,#09090b_0%,#a855f7_50%,#09090b_100%)] animate-[spin_2s_linear_infinite]"></div>
                <!-- 按钮主体 -->
                <div class="relative flex items-center gap-2 px-3 py-1.5 rounded-[7px] bg-[#09090b]/90 backdrop-blur-xl group-hover/btn:bg-[#09090b]/80 transition-colors border border-white/5">
                    <!-- 文字 -->
                    <span class="text-sd font-bold bg-gradient-to-r from-purple-400 to-pink-300 bg-clip-text text-transparent group-hover/btn:text-white group-hover/btn:drop-shadow-[0_0_10px_rgba(168,85,247,0.8)] transition-all">
                        验证卡密
                    </span>
                    <!-- 图标 -->
                    <div class="relative">
                        <Zap :size="18" class="text-purple-500 group-hover/btn:text-pink-300 transition-colors" />
                        <Zap :size="18" class="absolute inset-0 text-white opacity-0 group-hover/btn:animate-ping" />
                    </div>
                </div>
            </div>
        </button>
  ...
  <div class="flex items-center">
    ...
    <div class="flex-1 mx-4 flex items-center justify-between">
      ...
      <button class="group/edit relative px-4 py-1.5 rounded-lg bg-transparent overflow-hidden transition-all hover:shadow-[0_0_15px_rgba(168,85,247,0.3)]">
         <div class="absolute inset-0 border border-white/10 rounded-lg group-hover/edit:border-purple-400/50 transition-colors"></div>
         <div class="absolute inset-0 bg-purple-500/0 group-hover/edit:bg-purple-500/10 transition-colors"></div>
         <div class="flex items-center gap-2 relative z-10 text-gray-400 group-hover/edit:text-purple-300 transition-colors text-sd font-bold">
            <span>编辑提示词</span>
            <ArrowRight :size="18" class="opacity-0 -translate-x-2 group-hover/edit:opacity-100 group-hover/edit:translate-x-0 transition-all duration-300" />
         </div>
      </button>
    </div>
  </div>
        ...
        <!-- 第三行: 角色生图 (Pink Theme) -->
      <div class="bg-[#131318]/80 backdrop-blur-sm rounded-xl p-4 border border-white/5 hover:border-pink-500/40 hover:bg-[#131318] transition-all duration-300 group/item hover:shadow-[0_0_15px_-3px_rgba(236,72,153,0.15)]">

  <div class="flex items-center justify-between mb-4">
    <span class="text-gray-400 text-sd font-bold w-20 shrink-0 group-hover/item:text-pink-300 transition-colors">角色生图：</span>

    <div class="flex-1 mx-4 flex items-center gap-2">
      <div class="flex-1 flex items-center justify-between bg-[#0a0a0f] border border-white/10 rounded-lg px-3 py-2.5 cursor-pointer hover:border-pink-500/50 hover:shadow-[0_0_10px_rgba(236,72,153,0.15)] transition-all group/select">
        <div class="flex items-center gap-2">
          <Sparkles :size="18" class="text-pink-400 group-hover/select:animate-pulse"/>
          <span class="text-pink-100 text-sd font-bold">Banana Images</span>
        </div>
        <ChevronDown :size="18" class="text-gray-500 group-hover/select:text-pink-400 transition-colors" />
      </div>
    </div>
  </div>
  ...
  <div class="flex items-center">
    ...
    <div class="flex-1 mx-4 flex items-center justify-between">
      ...
      <button class="group/edit relative px-4 py-1.5 rounded-lg bg-transparent overflow-hidden transition-all hover:shadow-[0_0_15px_rgba(236,72,153,0.3)]">
         <div class="absolute inset-0 border border-white/10 rounded-lg group-hover/edit:border-pink-400/50 transition-colors"></div>
         <div class="absolute inset-0 bg-pink-500/0 group-hover/edit:bg-pink-500/10 transition-colors"></div>
         <div class="flex items-center gap-2 relative z-10 text-gray-400 group-hover/edit:text-pink-200 transition-colors text-sd font-bold">
            <span>编辑提示词</span>
            <ArrowRight :size="12" class="opacity-0 -translate-x-2 group-hover/edit:opacity-100 group-hover/edit:translate-x-0 transition-all duration-300" />
         </div>
      </button>
...
<!-- 🔥 终极交互：全局保存按钮 (放在所有卡片的下方) 🔥 -->
<div class="w-full flex justify-center mt-8 pb-2">

  <button class="relative group/save cursor-pointer active:scale-95 transition-transform duration-200">

    <!-- 1. 核心爆发光晕 (Hover时三色混合炸裂) -->
    <div class="absolute -inset-1 bg-gradient-to-r from-cyan-500 via-purple-500 to-pink-500 rounded-full blur opacity-20 group-hover/save:opacity-100 group-hover/save:blur-md transition-all duration-500"></div>

    <!-- 2. 动态旋转的 RGB 流光边框 -->
    <div class="relative rounded-full p-[2px] overflow-hidden bg-[#09090b]">
        <!-- 这里的 conic-gradient 包含了三种主色调 -->
        <div class="absolute inset-[-1000%] bg-[conic-gradient(from_90deg_at_50%_50%,#09090b_0%,#06b6d4_25%,#a855f7_50%,#ec4899_75%,#09090b_100%)] animate-[spin_2s_linear_infinite]"></div>

        <!-- 3. 按钮内胆 -->
        <div class="relative flex items-center gap-3 px-10 py-3 rounded-full bg-[#09090b] group-hover/save:bg-[#0f0f12] transition-colors">

            <!-- 左侧装饰点 -->
            <div class="w-1.5 h-1.5 rounded-full bg-cyan-400 shadow-[0_0_5px_#22d3ee] group-hover/save:animate-pulse"></div>

            <!-- 文字：全息渐变色 -->
            <span class="text-base font-black tracking-[0.2em] uppercase bg-gradient-to-r from-cyan-300 via-purple-300 to-pink-300 bg-clip-text text-transparent group-hover/save:text-white transition-all duration-300">
                保存当前配置
            </span>

            <!-- 图标：带位移动画 -->
            <div class="relative overflow-hidden w-5 h-5 flex items-center justify-center">
                <Save :size="18" class="text-pink-400 absolute transition-all duration-300 group-hover/save:translate-y-[-150%] group-hover/save:scale-0" />
                <Check :size="20" class="text-emerald-400 absolute transition-all duration-300 translate-y-[150%] scale-0 group-hover/save:translate-y-0 group-hover/save:scale-110" />
            </div>

            <!-- 扫光特效 -->
            <div class="absolute inset-0 w-full h-full bg-gradient-to-r from-transparent via-white/10 to-transparent skew-x-[-30deg] translate-x-[-150%] group-hover/save:translate-x-[150%] transition-transform duration-700 ease-in-out"></div>
        </div>
    </div>
  </button>
...
<!-- 需要引入的图标 (假设使用 lucide-vue-next) -->
<!-- import { Sliders, History, ChevronDown, Sparkles, Key, Zap, CheckCircle2, ArrowRight } from 'lucide-vue-next'; -->
          ...
          <!-- 点击外部关闭遮罩 (透明) -->
           <div v-if="showModelConfig" @click="showModelConfig = false" class="fixed inset-0 z-[90]"></div>
        </div>
      ...
      <div class="w-1/2 h-full flex flex-col border-r border-white/5 bg-gradient-to-b from-[#0f0f15]/90 via-[#0a0a10]/80 to-[#0f0f15]/90 backdrop-blur-xl relative z-10 transition-all duration-300">
        ...
        <div class="h-14 px-6 border-b border-white/5 flex items-center justify-between shrink-0 bg-gradient-to-r from-[#111118]/90 to-[#0a0a12]/90 relative">
         <div class="flex items-center gap-4 select-none">
          ...
          <!-- 标题 -->
          <span class="text-2xl font-black bg-gradient-to-r from-cyan-200 via-purple-200 to-cyan-200 bg-[length:200%_auto] animate-gradient-flow bg-clip-text text-transparent drop-shadow-[0_0_15px_rgba(34,211,238,0.25)] tracking-wider">
            二创动态漫：{{ projectName || '未命名' }} ({{ clips.length }})
          </span>
        </div>

<div class="relative group/search z-20 mr-2">

    <div class="absolute -inset-1 bg-gradient-to-r from-cyan-600/50 via-blue-500/50 to-cyan-400/50 rounded-lg blur opacity-0 group-focus-within/search:opacity-100 transition-opacity duration-500"></div>

    <div class="relative flex items-center bg-[#05050a] border border-white/10 group-focus-within/search:border-cyan-500/50 rounded-lg overflow-hidden transition-all duration-300 shadow-[0_4px_10px_rgba(0,0,0,0.5)]">

        <div class="pl-2.5 pr-1 text-gray-600 group-focus-within/search:text-cyan-400 transition-colors duration-300">
             <Search :size="14" class="group-focus-within/search:animate-pulse" />
        </div>

        <input
            v-model="searchClipId"
            @keyup.enter="handleSearchClip"
            type="number"
            placeholder="分镜跳转"
            class="w-20 focus:w-28 bg-transparent border-none outline-none focus:ring-0 text-sm font-bold text-cyan-50 placeholder-gray-700 py-1.5 font-mono tracking-widest transition-[width] duration-300 ease-out"
        />

        <div class="absolute bottom-0 left-0 h-[1px] w-0 bg-cyan-400 shadow-[0_0_8px_#22d3ee] group-focus-within/search:w-full transition-all duration-500 ease-out"></div>

        <div class="absolute top-0 right-0 w-1.5 h-1.5 border-t border-r border-cyan-500/30 opacity-0 group-focus-within/search:opacity-100 transition-opacity"></div>

        <div
            v-if="searchClipId"
            class="mr-1 animate-in slide-in-from-right-2 fade-in duration-200"
        >
            <button
              @click="handleSearchClip"
              class="flex items-center justify-center p-1 rounded bg-cyan-900/30 hover:bg-cyan-500 text-cyan-400 hover:text-white border border-cyan-500/30 transition-all active:scale-95"
            >
              <ArrowRight :size="12" />
            </button>
        </div>
        ...
        <!-- 分镜卡片列表 -->
        <div ref="clipListRef" class="flex-1 overflow-y-auto custom-scroll p-6 space-y-5">
         <div
  v-for="(clip, index) in clips"
  :key="clip.id"
  :id="'clip-card-' + index"   @click="selectClip(index)"
  :class="[
...
>
            ...
            <div class="flex justify-between items-center border-b border-white/5 pb-3">
              <div class="flex items-center gap-3">
                ...
                <span class="text-[10px] px-2 py-1 rounded-lg bg-gradient-to-r from-cyan-900/30 to-purple-900/30 text-cyan-300 border border-cyan-500/20">15s</span>
              </div>
              ...
              <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                <button
                  ...
                  class="relative p-2 rounded-lg bg-gradient-to-br from-[#0a0a10] to-[#05050a] border border-purple-500/20 hover:border-purple-400/40 text-purple-400 hover:text-purple-300 transition-all hover:shadow-[0_0_10px_rgba(168,85,247,0.2)] group/edit"
                  title="详情编辑"
                >
                  <FileEdit :size="20" class="group-hover/edit:scale-110 transition-transform"/>
                </button>

                <div class="w-[1px] h-4 bg-gradient-to-b from-transparent via-white/10 to-transparent mx-1"></div>
                ...
                <button @click.stop="addClip(index, 0)" class="relative p-2 rounded-lg bg-gradient-to-br from-[#0a0a10] to-[#05050a] border border-cyan-500/20 hover:border-cyan-400/40 text-cyan-300 hover:text-white transition-all hover:shadow-[0_0_10px_rgba(34,211,238,0.2)] group/up">
                  <ArrowUp :size="20" class="group-hover/up:-translate-y-0.5 transition-transform"/>
                </button>
                ...
                <button @click.stop="addClip(index, 1)" class="relative p-2 rounded-lg bg-gradient-to-br from-[#0a0a10] to-[#05050a] border border-cyan-500/20 hover:border-cyan-400/40 text-cyan-300 hover:text-white transition-all hover:shadow-[0_0_10px_rgba(34,211,238,0.2)] group/down">
                  <ArrowDown :size="20" class="group-hover/down:translate-y-0.5 transition-transform"/>
                </button>
                ...
                <button @click.stop="deleteClip(index)" class="relative p-2 rounded-lg bg-gradient-to-br from-[#0a0a10] to-[#05050a] border border-red-500/20 hover:border-red-400/40 text-red-400 hover:text-red-300 transition-all hover:shadow-[0_0_10px_rgba(239,68,68,0.2)] group/delete">
                  <Trash2 :size="20" class="group-hover/delete:scale-110 transition-transform"/>
                </button>
              </div>
            </div>
        ...
        <div class="h-[55%] border-b border-white/5 flex flex-col bg-gradient-to-b from-[#0f0f15] to-[#0a0a10]">
          <div class="h-14 px-6 flex items-center justify-between border-b border-white/5 bg-gradient-to-r from-[#111118]/50 to-[#0a0a12]/50">
           <h2 class="text-xl font-black flex items-center gap-4 group/title cursor-default select-none">
              ...
              <!-- 标题 -->
              <span class="text-2xl font-black bg-gradient-to-r from-cyan-200 via-purple-200 to-cyan-200 bg-[length:200%_auto] animate-gradient-flow bg-clip-text text-transparent drop-shadow-[0_0_15px_rgba(34,211,238,0.25)] tracking-wider">
                视频总监：分镜预览 {{ String(activeClipIndex + 1).padStart(2, '0') }}
              </span>
            </h2>
          </div>
...
<div v-if="showQuickEditModal && currentEditClip" class="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 animate-in fade-in duration-200">

      <div class="absolute inset-0" @click="showQuickEditModal = false"></div>

      <div class="relative w-[1000px] h-[100vh] bg-[#0a0a0f] border border-purple-500/30 rounded-2xl shadow-[0_0_50px_rgba(168,85,247,0.2)] flex flex-col overflow-hidden group/modal">

        <div class="absolute inset-0 pointer-events-none rounded-2xl border border-purple-500/20"></div>

        <div class="h-16 px-6 border-b border-white/10 bg-[#111118] flex items-center justify-between shrink-0">
          <div class="flex items-center gap-4">
             <div class="p-2 bg-purple-500/20 rounded-lg border border-purple-500/30">
                <FileEdit :size="24" class="text-purple-400"/>
             </div>
             <div>
                <h3 class="text-xl font-black text-white tracking-wide flex items-center gap-2">
                  分镜脚本编辑总监:<span class="text-purple-500">第{{ String(currentEditClip.index + 1).padStart(2, '0') }}条分镜</span>
                </h3>
                <p class="text-xl font-bold text-gray-500 font-mono">VERTICAL EDITING MODE</p>
             </div>
          </div>
          <button @click="showQuickEditModal = false" class="p-2 rounded-full hover:bg-white/10 text-gray-400 hover:text-white transition-colors cursor-pointer">
            <X :size="24" />
          </button>
        </div>
        ...
        <div class="flex-1 overflow-hidden flex flex-col">
         ...
         <div class="h-[30%] flex divide-x divide-white/5 bg-[#0a0a0f] shrink-0">

             <div class="w-1/2 p-4 flex flex-col gap-2">
                <label class="text-xl font-bold text-cyan-400 uppercase tracking-wider flex items-center gap-2">
                   <div class="w-1.5 h-1.5 bg-cyan-400 rounded-full animate-pulse"></div>
                   文案分镜脚本 (Script)
                </label>
                <textarea
                  v-model="currentEditClip.script"
                  class="flex-1 w-full bg-[#05050a] border border-white/10 rounded-xl p-3 text-sd font-bold leading-relaxed text-gray-200 focus:border-cyan-500/50 focus:ring-0 outline-none resize-none custom-scroll transition-all"
                  placeholder="请您在此编辑文案"
                ></textarea>
             </div>

             <div class="w-1/2 p-4 flex flex-col gap-2 bg-[#0c0c12]">
                <label class="text-xl font-bold text-pink-400 uppercase tracking-wider flex items-center gap-2">
                   <div class="w-1.5 h-1.5 bg-pink-400 rounded-full animate-pulse"></div>
                   分镜视频预览(VIDEO PREVIEW)
                </label>

                <div class="flex-1 w-full relative group/video border border-white/10 rounded-xl overflow-hidden bg-black shadow-lg">
                    <video
                      v-if="currentEditClip.originalThumb"
                      :src="currentEditClip.originalThumb"
                      class="w-full h-full object-contain"
                      controls
                      autoplay
                      loop
                    ...
                    <div v-else class="w-full h-full flex items-center justify-center flex-col gap-3 text-gray-600">
                       <div class="p-2 bg-white/5 rounded-full border border-white/5">
                         <Video :size="18" class="opacity-50"/>
                       </div>
                       <span class="text-[10px] font-mono tracking-widest">NO SOURCE</span>
                    </div>

                    <div class="absolute top-2 left-2 px-2 py-1 bg-black/60 backdrop-blur rounded text-[10px] text-pink-300 font-bold border border-pink-500/30 flex items-center gap-2 pointer-events-none">
          ...
          <div class="flex-1 p-5 flex flex-col gap-3 bg-[#0a0a0f] border-t border-white/10">
             <label class="text-xl font-bold text-purple-400 uppercase tracking-wider flex items-center gap-2">
                <div class="w-1.5 h-1.5 bg-purple-400 rounded-full animate-pulse"></div>
                推理提示词 (Prompt)
             </label>
            <textarea
               v-model="currentEditClip.prompt"
               class="flex-1 w-full h-full bg-[#05050a] border border-white/10 rounded-xl p-4 text-sd font-bold leading-relaxed text-gray-200 focus:border-purple-500/50 focus:ring-0 outline-none resize-none custom-scroll transition-all"
               placeholder="请您在此编辑优化提示词"
             ></textarea>
          </div>
...
import { useConfirm } from '../utils/confirm'; // 引入 hook

// 获取 confirm 方法
const { confirm } = useConfirm(); // 注意这里解构出 confirm 方法
// 2. 🔥 修改这里：使用相对路径 "../" 返回上一级找到 utils
import { useToast } from '../utils/toast';
import GlobalConfirm from './GlobalConfirm.vue'; // 新增
import {
...
const clipListRef = ref<HTMLElement | null>(null);
...
const currentEditClip = ref<Clip | null>(null); // 当前正在编辑的分镜对象

// 打开弹窗的函数
const openEditModal = (index: number) => {
  // 直接引用 clips 数组里的对象，利用 Vue 的响应式实现自动同步
  currentEditClip.value = clips.value[index];
  showQuickEditModal.value = true;
};


// 🔍 分镜搜索相关逻辑
const searchClipId = ref(''); // 绑定输入框

// 修改 handleSearchClip 函数
const handleSearchClip = () => {
  if (!searchClipId.value) return;

  const targetId = parseInt(searchClipId.value);
  if (isNaN(targetId)) return toast.warning("请输入有效的分镜ID数字");

  const targetIndex = targetId - 1; // 假设 ID 对应 Index+1

  if (targetIndex >= 0 && targetIndex < clips.value.length) {
    // 1. 选中分镜
    selectClip(targetIndex);

    // 2. DOM 滚动逻辑 (修改部分)
    // 等待 DOM 更新后执行，确保 active 样式已应用（可选，但更稳妥）
    setTimeout(() => {
        const cardElement = document.getElementById(`clip-card-${targetIndex}`);
        const container = clipListRef.value;
        ...
        if (cardElement && container) {
            ...
            // --- ✅ 替换为精准计算内部滚动 ---
            // 计算公式：子元素相对父元素的顶部偏移 - 父元素高度的一半 + 子元素高度的一半
            // 这样可以完美居中，且绝不会影响整个页面
            const topPos = cardElement.offsetTop;
            container.scrollTo({
                top: topPos - container.clientHeight / 2 + cardElement.clientHeight / 2,
                behavior: 'smooth'
            });

            toast.success(`已跳转到分镜 ${targetId}`);
        }
    }, 10); // 微小的延迟确保 DOM 获取准确

  } else {
    toast.error(`未找到分镜 ID: ${targetId}`);
  }
  ...
  try {
    ...
    if (resData.status === 'success') {
      if (!resData.data || resData.data.length === 0) {
       toast.warning("未能提取到角色，请检查文案是否包含人物描述。");
      } else {
        ...
        toast.success(`提取成功！共找到 ${newCharacters.length} 个角色。`);
      }
    } else {
      toast.error("提取失败: " + resData.msg);
    }
  } catch (error) {
    console.error("提取角色异常:", error);
    toast.error("网络请求错误，请检查后端服务是否启动。");
  } finally {
...
const saveToLibrary = (char: any) => {
  ...
  if (!char.description && !char.image) {
    toast.warning("该角色信息为空（无描述或图片），无法保存到档案库！");
    return;
  }
...
// C. 删除视频
const removeCharacterVideo = async (char: any) => {
  // 使用自定义确认框
...
const generateSingleCharacterVideo = async (char: any, silent = false) => {
  // 1. 校验前置条件
  if (!char.image) {
    return toast.warning("请先生成或上传角色图片，才能生成动态视频！");
  }
  if (!char.description) {
    return toast.warning("角色缺少特征描述/内容，无法生成！");
  }

// 仅在非静默模式下弹窗确认
  if (!silent) {
    const isConfirmed = await confirm(
      `确定要为角色【${char.label}】生成Sora动态视频吗？\n生成过程可能需要几分钟。`, // 内容
      "视频生成确认", // 标题
      "info" // 类型：青色 (适合生成/创建操作)
    );
  ...
  try {
    ...
    if (!response.ok) {
      ...
      toast.error(`提交任务失败 (Code ${response.status}):\n${errMsg}`);
      ...
      const pollInterval = setInterval(async () => {
        try {
          ...
          if (statusData.status === 'success' || statusData.status === 'completed') {
            ...
            char.isGeneratingVideo = false;
            if (!silent) toast.success(`🎉 角色【${char.label}】动态视频生成成功！`);
          ...
          // --- 情况 B: 失败 ---
            ...
            clearInterval(pollInterval);
            char.isGeneratingVideo = false;
            toast.error(`❌ 生成失败: ${statusData.msg}`);
          }
...
// 2. 删除图片逻辑
// 删除图片逻辑
const removeCharacterImage = async (char: any) => {
  const isConfirmed = await confirm(
    "确定要移除这张图片吗？\n操作不可撤销，移除后需重新上传。", // 内容
    "移除图片确认", // 标题
    "danger" // 类型：红色警示
...
const generateSingleCharacterImage = async (char: any, signal?: AbortSignal) => {
  if (!char.description) return toast.warning("该角色没有特征描述，无法生成！");
...
const batchGenerateImages = async () => {
  ...
  if (targetChars.length === 0) {
    ...
    if (checkedCount > 0) {
       return toast.warning(`已勾选 ${checkedCount} 个角色，但它们要么已有图片，要么缺少描述词。\n批量生成会自动跳过已有图片的角色。`);
    } else {
       return toast.warning("请先勾选需要生成的角色！");
    }
  }
  ...
  // 使用 await 等待用户点击
const isConfirmed = await confirm(
  `即将为 ${targetChars.length} 个“无图”角色生成图片，确定继续吗？`, // 内容
  '批量任务确认', // 标题
  'warning' // 类型：warning (黄色), info (青色), danger (红色)
);
...
if (!isConfirmed) return;
  ...
  try {
    ...
    if (!batchImageController?.signal.aborted) {
      toast.success(`批量任务完成！共生成 ${targetChars.length} 张图片。`);
    }
  } catch (err) {
...
const deleteClip = async (index: number) => {
  // 使用自定义确认框
  const isConfirmed = await confirm(
    '确定要删除这个分镜吗？\n该分镜的文案、原视频及生成记录将被永久移除。', // 内容
    '删除分镜确认', // 标题
    'danger' // 类型：红色警示
  );
      ...
      if (data.status === 'success' && data.job_id) {
        ...
        pollInterval = setInterval(async () => {
          ...
          try {
            ...
            if (statusData.progress) clip.progress = statusData.progress;
        ...
        cleanup();
        toast.error(`启动失败: ${data.msg}`);
        resolve();
      }
    } catch (error: any) {
      cleanup();
      if (error.name !== 'AbortError') toast.error("网络请求错误");
      resolve();
    }
  });
};
...
const generateVideo = (index: number) => {
  ...
  if (!clip.prompt) return toast.warning("请先输入提示词！");

  // 防止重复点击
  if (clip.isGenerating) return toast.warning("任务进行中...");

  // 单个生成不需要 AbortController，直接调用
  // 增加 catch 防止报错导致状态卡死
  processClipVideo(clip).catch(() => {
      clip.isGenerating = false;
  });
};
...
const batchGenerateVideos = async () => {
  ...
  if (targetClips.length === 0) {
    return toast.warning("没有检测到可生成的任务（需要有提示词且未在生成中）！");
  }

// 3. 自定义确认弹窗
  const isConfirmed = await confirm(
    `即将批量生成 ${targetClips.length} 个视频。\n过程耗时较长，请保持浏览器开启，是否继续？`, // 内容 (加了换行让排版更美观)
    '批量生成确认', // 标题
    'info' // 类型：青色 (代表生成/启动)
  );
...
const rewriteScript = async (index: number) => {
    const clip = clips.value[index];
    if (!clip.script) return toast.warning("请先输入文案");
    ...
    try {
        ...
        toast.error("连接服务器失败，请确保 ai_server.py 已运行");
    } finally {
...
const optimizePrompt = async (index: number) => {
    const clip = clips.value[index];
    if (!clip.originalPath) return toast.warning("该分镜没有关联的原视频，无法进行推理！");
    ...
    try {
        ...
        if (data.success) {
           ...
           toast.error(`推理失败: ${data.msg}`);
        }
    } catch (error) {
        console.error("API Error:", error);
        toast.error("连接服务器失败，请确保后端服务已启动且网络正常。");
    } finally {
...
const batchInferScripts = async () => {
  ...
  if (targetClips.length === 0) return toast.warning("没有检测到需要润色的文案（原文为空或已润色）！");
  ...
  const signal = batchRewriteController.signal;

  // 3. 定义单个任务 Worker
  const worker = async (clip: Clip) => {
    // 【核心】双重检查：如果用户点了停止，直接跳过后续逻辑
    if (signal.aborted || !isBatchRewriting.value) return;

    clip.isRewriting = true;
    try {
      // 传递 signal 给 fetch，这样调用 abort() 时请求会立即断开
      const response = await fetch('http://127.0.0.1:8000/rewrite', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ text: clip.script }),
      ...
      if (data.success) {
        ...
        clip.showOriginal = false;
         ...
         console.warn(`Clip ${clip.index} rewrite failed: ${data.msg}`);
      }
    } catch (err: any) {
      // 忽略因“停止”导致的中断错误，只打印真正的网络错误
      if (err.name !== 'AbortError') {
        console.error(`Script error clip ${clip.index}`, err);
      }
    } finally {
      clip.isRewriting = false;
      // 只有在未停止的情况下才更新进度条
      if (!signal.aborted && isBatchRewriting.value) {
  ...
  try {
    // 5并发执行 (如果觉得卡顿可以把 5 改小，比如 3)
    await asyncPool(5, targetClips, worker);

    // 只有正常跑完（没被中断）才提示完成
    if (!signal.aborted) {
        toast.success("批量改文完成！");
    }
  } catch (error) {
     // 捕获 asyncPool 可能抛出的错误
     console.log("Batch process stopped or failed");
  } finally {
    // 确保最后状态复位
    isBatchRewriting.value = false;
    batchRewriteController = null;
...
// ----------------- 2. 批量推理提示词逻辑 -----------------
// ================= 2. 批量推理提示词逻辑 (修复版：支持立即中断) =================

// 停止函数
const stopBatchOptimize = () => {
  // 1. 发送中断信号，正在进行的请求会立即断开
  ...
  toast.info("已停止批量推理任务");
};

// 开始函数
const batchInferPrompts = async () => {
  // 1. 筛选：有视频路径 + 有文案 + 但还没生成提示词
  const targetClips = clips.value.filter(c => c.originalPath && c.script && !c.prompt);

  if (targetClips.length === 0) {
    // 检查一下是不是因为没有视频
    const noVideoCount = clips.value.filter(c => !c.originalPath).length;
    if (noVideoCount > 0) {
        return toast.warning(`检测到 ${noVideoCount} 个分镜没有关联视频，无法推理。请先上传视频或拆帧。`);
    }
    return toast.warning("所有分镜都已完成推理，或缺少文案/视频！");
  }
  ...
  const signal = batchOptimizeController.signal;

  // 3. 定义单个 Worker
  const worker = async (clip: Clip) => {
    // 【核心】双重检查：如果停止了，直接跳过
    if (signal.aborted || !isBatchOptimizing.value) return;
    ...
    try {
      ...
      if (data.success) {
        clip.prompt = data.data;
         ...
         console.warn(`Clip ${clip.index} optimize failed: ${data.msg}`);
         // 可选：单个失败不打断整体，但可以给个小提示
         // toast.error(`分镜 ${clip.index + 1} 推理失败`);
      }
    } catch (err: any) {
      // 忽略手动停止导致的错误
      if (err.name !== 'AbortError') {
         console.error(`Prompt error clip ${clip.index}`, err);
      }
    } finally {
      clip.isOptimizing = false;
      // 只有未停止时才更新进度
      if (!signal.aborted && isBatchOptimizing.value) {
  ...
  try {
    // 4. 并发执行 (视觉推理比较慢，建议并发数设为 3，太高容易被阿里云限流)
    await asyncPool(3, targetClips, worker);

    // 只有完整跑完才提示成功
    if (!signal.aborted) {
       toast.success("批量推理提示词完成！");
    }
  } catch (error) {
     console.log("Batch optimization process stopped");
  } finally {
    isBatchOptimizing.value = false;
    batchOptimizeController = null;
  }
...
const batchGenerateCharacterVideos = async () => {
  ...
  if (checkedChars.length === 0) {
    return toast.warning("请先勾选需要生成视频的角色！");
  }
  ...
  // 4. 校验与提示
  if (targetChars.length === 0) {
    return toast.warning(`已勾选 ${checkedChars.length} 个角色，但它们全部缺少图片或描述！\n\n请先上传或生成图片。`);
  }
...
// 4. 构造确认信息
  ...
  const worker = async (char: any) => {
    ...
    // 由于 generateSingleCharacterVideo 原函数里有 toast.success 且是异步轮询，
    ...
    // 这里我们手动实现一个不带 toast.success 的精简版生成逻辑，或者直接调用原函数
...
const batchMatchCharacterIds = async () => {
  ...
  const checkedChars = characterList.value.filter(c => c.checked);
  if (checkedChars.length === 0) {
    return toast.warning("请先勾选需要匹配 ID 的角色！");
  }
  ...
  if (targetChars.length === 0) {
    ...
    if (alreadyDoneCount === checkedChars.length) {
      return toast.warning("所选角色都已经拥有角色ID了，无需重复匹配。");
    } else {
      return toast.warning(`已勾选 ${checkedChars.length} 个角色，但无可匹配项。\n\n原因可能是：\n1. 角色已有ID (自动跳过)\n2. 缺少生成任务ID (请先生成视频，且不要刷新页面)`);
    }
  }
 ...
 // 6. 定义单个匹配任务 Worker
  const worker = async (char: any) => {
    try {
      console.log(`🚀 [开始匹配] ${char.label} (TaskID: ${char.taskId})`);

      // 设置 fetch，注意：如果是 248s 这么久，前端不要设置 signal 超时，让他一直等
      ...
      if (!response.ok) {
         console.warn(`❌ [网络失败] ${char.label}: ${response.status}`);
         toast.error(`请求失败: ${char.label} (超时或网络错误)`);
         return;
      }
      ...
      if (validData && validData.username) {

        // 1. 更新数据
        char.name = '@' + validData.username;
        char.link = validData.permalink || `https://sora.chatgpt.com/profile/${validData.username}`;

        console.log(`✅ [匹配成功了] ${char.label} -> ${char.name}`);

        // 🔥🔥🔥【核弹级修复】强制触发 Vue 更新 🔥🔥🔥
        // 有时候在 asyncPool 里修改深层对象，Vue 反应不过来，这里强制刷新一下列表引用
        characterList.value = [...characterList.value];

      } else {
        const msg = validData.msg || "未返回 username";
        console.warn(`⚠️ [数据缺失] ${char.label}: ${msg}`, res);
      }

    } catch (err) {
      console.error(`❌ [异常] ${char.label}:`, err);
      // 如果是因为 248s 导致的超时，这里会打印出来
      toast.error(`匹配异常: ${char.label} (可能是处理时间过长超时)`);
    }
  };
  ...
  try {
    ...
    toast.success("批量匹配流程结束！");

  } catch (e) {
    console.error("并发调度异常", e);
    toast.error("匹配过程发生错误，请检查控制台");
  } finally {
...
// 2. 清空所有角色
// 清空所有角色
const clearAllCharacters = async () => {  // <--- 🔥 必须加上 async 关键字
  if (characterList.value.length === 0) {
    return toast.success("当前没有角色可清除。");
  }

  // 二次确认，防止误操作毁灭世界
  const isConfirmed = await confirm(
    "⚠️ 高危操作警告 ⚠️\n\n确定要【清空所有】角色数据吗？\n此操作将丢失所有已提取的描述、生成的图片和视频链接，且无法撤销！", // 内容
    "清空全部确认", // 标题
    "danger" // 类型：红色警示 (最高级别)
  );
...
.animate-gradient-xy {
  background-size: 200% 200%;
  animation: gradient-xy 3s ease infinite; /* 加快了速度，更炫酷 */
}
/* 🔥 核心修复：隐藏浏览器默认的数字输入框上下箭头 */
